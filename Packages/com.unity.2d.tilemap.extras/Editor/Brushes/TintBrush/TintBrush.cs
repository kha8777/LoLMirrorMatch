...
namespace UnityEditor.Tilemaps
{
    ...
    public class TintBrushEditor : GridBrushEditorBase
    {
        ...
        // cached valid targets to avoid repeated scene-wide searches
        private static GameObject[] s_cachedTargets;

        private void OnEnable()
        {
            // Invalidate cache when hierarchy changes in the editor
        ...
        public override GameObject[] validTargets
        {
            get
            {
                if (s_cachedTargets != null)
                    return s_cachedTargets;

                // Use the existing API but suppress the obsolete warning so this file compiles
                // across Unity versions where FindObjectsOfType is marked obsolete.
                #pragma warning disable CS0618
                s_cachedTargets = GameObject.FindObjectsOfType<Tilemap>().Select(x => x.gameObject).ToArray();
